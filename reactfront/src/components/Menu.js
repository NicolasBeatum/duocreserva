            ...
            <div className="card-body">
              ...
              {cart.map(item => (
                <div key={item.ID_Comida || item.ID_Postre || item.ID_Ensalada || item.ID_Jugo} className="d-flex justify-content-between align-items-center mb-2">
                  ...
                  <div>
                    <button className="btn btn-sm btn-outline-danger me-2" onClick={() => removeFromCart(item.ID_Comida || item.ID_Postre || item.ID_Ensalada || item.ID_Jugo)}>Eliminar</button>
                  </div>
                </div>
              ))}
              <hr />
